...
def add_entry():
    ...
    new_entry = pd.DataFrame(
        {"Pokemon": [pokemon], "Location": [location], "Date": [date]}
    )
...
def leaderboard():
    ...
    counts["Last Time Ran"] = counts["Last Time Ran"].str.replace(
        r"\b0(\d)", r"\1", regex=True
    )
...
def last10():
    ...
    last_10_entries["Date"] = last_10_entries["Date"].str.replace(
        r"\b0(\d)", r"\1", regex=True
    )
